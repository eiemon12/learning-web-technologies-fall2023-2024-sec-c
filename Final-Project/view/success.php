<?php
    ...
    require_once('../model/successModel.php');

    if(isset($_REQUEST['success'])){
        
        $success=$_REQUEST['success'];
        $value = insertSuccess($success);
        if($value){
...
<head>
    ...
    <script>
        function validateSuccessForm() {
            var successContent = document.forms["successForm"]["success"].value;
            if (successContent == "") {
                alert("Success content must be filled out");
                return false;
            }
        }
    </script>
    ...
    <table width="100%" height="100%" cellspacing="0" cellpadding="0" border="1">
        ...
        <tr>
            ...
            <td>
                <h3>Post Success</h3>
                <h3>
                    <form action="success.php" method="post" name="successForm" onsubmit="return validateSuccessForm()">
                        <textarea rows="2" name="success" placeholder="Post success here.."></textarea><br>
                        <input type="submit" value="Submit">
                    </form>
                </h3>
                <h2>Total Successes</h2>
                <?php
                $con = getConnection();
                $sql = "select * from successinfo";
                $result = mysqli_query($con, $sql);
                while ($row = mysqli_fetch_array($result)) {
                    echo "<table width='100%'><tr><td width='5%'>" . $row['sl'] . "</td><td>" . $row['success'] . "</td></tr></table>";
                }
                ?>
            </td>
        </tr>
